...
var Aircraft = Fiber.extend(function () {
    ...
    return {
        ...
        matchCallsign: function (callsign) {
            ...
            if (this_callsign.indexOf(callsign) == 0)
                return true;
            return false;
        },
        ...
        getRadioCallsign: function (condensed) {
            ...
            if (cs == "November")
                ...
                cs += " " + groupNumbers(callsign) + heavy;
            return cs;
        },
        getClimbRate: function () {
            ...
            if (this.model.engines.type == "J")
                var serviceCeilingClimbRate = 500;
            else
                var serviceCeilingClimbRate = 100;
        ...
        runCommand: function (command) {
            if (!this.inside_ctr)
                return true;
            var user_input = $.map(command.toLowerCase().split(" "), function (v) {
                if (v.length > 0)
                    return v;
            });
            var commands = [];
            ...
            for (var i in user_input) {  //fill commands[] based on user input
                ...
                if (is_shortCommand) {
                    ...
                    if (commands.length != 0)
                        commands[commands.length - 1].push(commandArguments); //append the arguments to previous command
        ...
        run: function (command, data) {
            ...
            if (this.COMMANDS[command]) {
                ...
                $.each(this.COMMANDS, function (k, v) {
                    if (v.synonyms && v.synonyms.indexOf(command) != -1) {
                        call_func = v.func;
                    }
                });
            }
        ...
        runAltitude: function (data) {
            ...
            function isExpedite(s) {
                if ((s.length >= 1 && "expedite".indexOf(s) == 0) || s == "x")
                    return true;
                return false;
            }
        ...
        runHold: function (data) {
            var turn = '';
            if ("left".indexOf(data) == 0 && data.length >= 1)
                turn = "left";
            else if ("right".indexOf(data) == 0 && data.length >= 1)
                turn = "right";
            else
                return ["fail", "hold direction not understood"];
        ...
        runDme: function (data) {
            var turn = '';
            if ("cw".indexOf(data) == 0 && data.length >= 1)
                turn = "left";
            else if ("ccw".indexOf(data) == 0 && data.length >= 1)
                turn = "right";
            else
                return ["fail", "DME Arc direction not understood", "say again"];
            ...
            if (this.isTakeoff())
                return ["ok", "after departure, will start DME Arc to " + this.requested.turn];

            return ["ok", "staring DME Arc to " + this.requested.turn + " at " + this.requested.altitude + " feet"];
        },
        ...
        runWait: function (data) {
            ...
            if (this.mode == "taxi")
                return ["fail", "already taxiing to " + radio_runway(this.fms.currentWaypoint().runway)];
            ...
            if (data) {
                if (!airport_get().getRunway(data.toUpperCase()))
                    return ["fail", "no runway " + data.toUpperCase()];
        ...
        runTakeoff: function (data) {
            ...
            if (this.mode == "taxi")
                return ["fail", "taxi to runway " + radio_runway(this.fms.currentWaypoint().runway) + " not yet complete"];
        ...
        runLanding: function (data) {
            ...
            if (!runway) {
                if (!data)
                    return ["fail", "runway not understood"];
                else
                    return ["fail", "there is no runway " + radio_runway(data)];
            }
        ...
        updateTarget: function () {
            ...
            if (this.fms.currentWaypoint().navmode == "rwy") {
                ...
                if (!runway.getILS(this.fms.currentWaypoint().runway) || !ils)
                    ils = 40;
...
//          ui_log(this.getRadioCallsign() + " passed over " + this.fms.currentWaypoint().name.toUpperCase() + ", will maintain heading " + heading_to_string(this.fms.currentWaypoint().heading) + " at " + this.fms.currentWaypoint().altitude + " feet");
            ...
            } else if (this.fms.currentWaypoint() == "dme") {
                if (this.fms.currentWaypoint().turn == "right") {
                    var dme_fix = this.fms.currentWaypoint().location,
                            fix = ((dme_fix) ? dme_fix : [0, 0]),
                            dme_vector = vsub(this.position, fix),
                            distance_to_fix = distance2d(this.position, fix);
                    this.target.heading = vradial(dme_vector) - Math.PI / 2;
                    this.target.turn = null;
                } else if (this.fms.currentWaypoint().turn == "left") {
                    var dme_fix = this.fms.currentWaypoint().location,
                            fix = ((dme_fix) ? dme_fix : [0, 0]),
        ...
        updateStrip: function () {
            ...
            } else if (this.fms.currentWaypoint().navmode == "dme") {
                heading.text("DME-" + this.fms.currentWaypoint());
                heading.addClass("hold");
            } else if (this.fms.currentWaypoint().navmode == "hold") {
